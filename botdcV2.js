const { Client, GatewayIntentBits, EmbedBuilder, ActionRowBuilder, ButtonBuilder, ButtonStyle, ChannelType, PermissionsBitField, ModalBuilder, TextInputBuilder, TextInputStyle, Collection } = require('discord.js');
...
// Cache in-memory (akan hilang saat bot restart)
const ticketCache = {
    activeTickets: new Collection(), // userId -> ticketData
    ticketCounter: new Collection(), // guildId -> nextTicketId
};

// Rate limiting
const rateLimits = new Collection();
const RATE_LIMIT = {
    createTicket: 30000, // 30 detik antar pembuatan ticket
    closeTicket: 10000,  // 10 detik antar penutupan
};
...
async function handleCreateTicketModal(interaction) {
    ...
    try {
        ...
        // Get next ticket ID for this guild
...
async function handleConfirmClose(interaction) {
    ...
    try {
        ...
        // Find and remove ticket from cache
        ...
        // Get close reason from previous embed
...
async function closeTicketCommand(message, args) {
    ...
    // Send closure message and process
    const closeEmbed = new EmbedBuilder()
        .setColor(colors.warning)
        .setTitle('🎫 Tiket Ditutup')
        .setDescription(`**Tiket #${ticket.ticketNumber}** telah ditutup`)
...
// Other commands (add, remove, rename, logs, cleanup, help)
...
async function cleanupTickets(message) {
    ...
    for (const channel of channels.values()) {
        try {
            ...
            cleaned++;
            // Rate limit deletions to avoid hitting Discord limits
...
// Remove hardcoded token - use environment variable
...
if (!token) {
    ...
    console.log('1. Buat file .env dengan DISCORD_TOKEN=token_anda');
    console.log('2. Atau set environment variable di Render');
    console.log('3. Pastikan bot memiliki permission yang cukup');
    console.log('='.repeat(50));
    process.exit(1);
}
